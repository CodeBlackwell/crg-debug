...
- **Source is bind-mounted; toolchain commands are containerized.** Every build/typecheck/test command is returned in its runnable form `docker run --rm -v <repo>:/work -v crg-deps-<slug>:/work/<depdir> -w /work crg-env-<slug> sh -lc '<cmd>'`. Because the source is bind-mounted, later fix/gate steps edit files on the host as normal and re-run the same containerized strings — no image rebuild per edit.
- **Classify every baseline failure.** `code` = the compiler/typechecker reached this repo's own source and found a real defect → a bug, seeded to the queue. `env` = failed for any *other* reason (missing tool/dep/system lib provisioning couldn't supply, a build command not applicable to this project type, Docker unavailable, sandbox/network limit) → NOT a bug. Any residual `env` failure ⇒ **unfarmable**: stop, hand off, seed nothing.
