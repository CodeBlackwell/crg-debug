...
4. **Baseline build + typecheck.** Run the discovered build + typecheck commands once now. Any failure is an objective contract violation → add it straight to the confirmed bug queue (a compile/type error is its own repro; no further verification needed). This catches build failures and type errors a static source read misses — e.g. an unexported type referenced by generated code.

Output: confirmed graph stats line + resolved scope + toolchain profile + any baseline build/typecheck failures.
...
1. **RED** — write a test that defines the *expected* (correct) result, then run it and confirm it FAILS for the right reason (the actual bug — not a typo or setup error). A test that passes before the fix means the bug isn't reproduced → do NOT edit; mark the finding *suspected (unconfirmed)*. This failing test IS the reproduction the false-positive guard requires.
