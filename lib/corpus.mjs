...
export function fetchCorpus(repoRoot, { maxPRs = 1000 } = {}) {
  ...
  // --slurp wraps each page as an element of one outer JSON array.
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!repoRoot) { process.stderr.write('usage: corpus.mjs fetch|split|inventory <repoRoot> [arg]\n'); process.exit(1) }
  const dir = corpusDir(repoRoot)
  if (cmd === 'fetch') {
  ...
  } else { process.stderr.write('usage: corpus.mjs fetch|split|inventory <repoRoot> [arg]\n'); process.exit(1) }
}
