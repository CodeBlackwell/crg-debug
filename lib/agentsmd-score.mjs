...
//   node agentsmd-score.mjs rules   <repoRoot>  (prints the compact indexed judged-rule list + counts)
//   node agentsmd-score.mjs score   <repoRoot>  (panel.json + ledger.json -> scores.json; prints a compact summary)
//   node agentsmd-score.mjs stamp   <repoRoot>  (fills ledger.scoring from scores.json, deterministically)
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!repoRoot) { process.stderr.write('usage: agentsmd-score.mjs holdout|rules|score|stamp <repoRoot>\n'); process.exit(1) }
  ...
  if (cmd === 'holdout') {
    ...
    // The compact indexed rule list judges credit against. CLI-owned so the judge prompt and
    // the scorer share ONE construction of the index space — an agent never round-trips the
    // ledger (a 100KB+ ledger through a model's structured return silently truncates).
    const ledger = JSON.parse(readFileSync(join(base, 'ledger.json'), 'utf8'))
    const judged = judgedRules(ledger)
    const ruleList = judged
      .map((r, i) => `${i}: [${r.scope}]${r.unverifiedCommand ? ' (unverified command)' : ''} ${r.rule}`)
      .join('\n')
    process.stdout.write(JSON.stringify({
      count: judged.length,
      rules: (ledger.rules || []).length,
      cut: (ledger.cut || []).length,
      unverified: judged.filter(r => r.unverifiedCommand).length,
      ruleList,
    }) + '\n')
  } else if (cmd === 'score') {
    ...
    // stdout is a COMPACT summary — the full scores.json stays on disk. The gate agent copies
    // this into its structured return; printing the full file made models mangle it.
    const summary = {
      ...
      kept: kept.length, cutZeroPredictive: cutRules.length, rescued: scores.rescued.length,
      topKept: [...kept]
        .sort((x, y) => (Number(!!x.restatement) - Number(!!y.restatement)) || ((y.coverage || 0) - (x.coverage || 0)))
        .slice(0, 15)
        .map(r => ({ rule: r.rule, scope: r.scope, category: r.category, coverage: r.coverage || 0, restatement: !!r.restatement })),
    }
    process.stdout.write(JSON.stringify(summary, null, 2) + '\n')
  } else if (cmd === 'stamp') {
    // Deterministically fill ledger.scoring from scores.json — replaces the read-modify-write
    // persist agent (another whole-ledger round trip through a model).
    const ledger = JSON.parse(readFileSync(join(base, 'ledger.json'), 'utf8'))
    const scores = JSON.parse(readFileSync(join(base, 'scores.json'), 'utf8'))
    ledger.scoring = {
      fileCoverage: scores.fileCoverage, applicable: scores.applicable, totalComments: scores.totalComments,
      holdoutTotal: scores.holdoutTotal, unjudged: scores.unjudged,
    ...
    process.stdout.write(JSON.stringify({ ok: true, kept: (scores.kept || []).length, cut: (scores.cutZeroPredictive || []).length }) + '\n')
  } else { process.stderr.write('usage: agentsmd-score.mjs holdout|rules|score|stamp <repoRoot>\n'); process.exit(1) }
}
